...
async function fetchTemplateCost(templateId) {
  ...
  console.log("Calling getPrepaidPackageTemplate with ID:", templateId);
    ...
    try {
      if (r.prepaidpackagetemplateid) {
        ...
        if (tpl?.cost != null) {
          ...
          console.log("→ cost for template", r.prepaidpackagetemplateid, "=", tpl.cost);
        } else {
          console.log("❌ No cost found for template", r.prepaidpackagetemplateid);
        }
