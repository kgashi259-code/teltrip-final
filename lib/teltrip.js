...
async function fetchPackagesFor(subscriberId) {
  ...
  const s = resp?.getSubscriber?.subscriber;
  const pkg = s?.subscriberPrepaidPackageList?.[0];
  ...
  return {
    ...
    tsactivationutc: pkg?.tsactivationutc ?? null,
    tsexpirationutc: pkg?.tsexpirationutc ?? null,
    pckdatabyte: pkg?.pckdatabyte ?? null,
    useddatabyte: pkg?.useddatabyte ?? null,
    subscriberOneTimeCost: Number.isFinite(cost) ? cost : null
  };
}
